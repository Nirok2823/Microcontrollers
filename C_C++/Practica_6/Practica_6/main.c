...
void Teclado(volatile char *cont, volatile uint8_t *reg_N, volatile uint8_t *fg){
...
uint8_t convercion_hx_dec(uint8_t hex){
	/*
	uint8_t decenas=hex&conservar_dec;
	uint8_t unidades=hex&conservar_uni;
	*/
	Swap(&hex);
	uint8_t aux_sum=0;
	while(hex>=0x10){
...
int main(void)
{
    ...
    {
		if (flag==0){
			Teclado(&contador, &reg_display, &flag);
			PORTC=reg_display;
		}
